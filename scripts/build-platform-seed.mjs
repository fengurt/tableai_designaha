...
for (const brand of brands) {
  ...
  statements.push(`INSERT INTO brand_records(slug,payload_json,version,created_at,updated_at) VALUES(${q(brand.slug)},${q(JSON.stringify(payload))},1,${q(timestamp)},${q(timestamp)}) ON CONFLICT(slug) DO UPDATE SET payload_json=excluded.payload_json,updated_at=excluded.updated_at;`);
  outbox(`seed-search-brand-${brand.slug}`, "search.index", "brand", brand.slug, 1, { entityType: "brand", entityId: brand.slug });
}
...
for (const name of await readdir(tempDir)) {
  if (/^platform-seed-\d+\.sql$/.test(name)) await rm(join(tempDir, name));
}
const chunks = [];
for (let index = 1; index < statements.length; index += 10) chunks.push(["PRAGMA foreign_keys = ON;", ...statements.slice(index, index + 10)]);
for (const [index, chunk] of chunks.entries()) await writeFile(join(tempDir, `platform-seed-${String(index + 1).padStart(3, "0")}.sql`), `${chunk.join("\n")}\n`);
console.log(JSON.stringify({ brands: brands.length, assets: assets.count, statements: statements.length, chunks: chunks.length }, null, 2));
